...
from projectqtemp.ops import _fermion_operator as fermion_operators
...
def uccsd_operator(single_amplitudes, double_amplitudes, anti_hermitian=True):
    ...
    uccsd_operator = fermion_operators.FermionOperator((), 0.0)
    ...
    for i, j in itertools.product(range(n_orbitals), repeat=2):
        ...
        uccsd_operator += fermion_operators. \
            FermionOperator(((i, 1), (j, 0)), single_amplitudes[i, j])
        if (anti_hermitian):
            uccsd_operator += fermion_operators. \
                FermionOperator(((j, 1), (i, 0)), -single_amplitudes[i, j])
    ...
    for i, j, k, l in itertools.product(range(n_orbitals), repeat=4):
        if double_amplitudes[i, j, k, l] == 0.:
            continue
        uccsd_operator += fermion_operators. \
            FermionOperator(((i, 1), (j, 0), (k, 1), (l, 0)),
                            double_amplitudes[i, j, k, l])
        if (anti_hermitian):
            uccsd_operator += fermion_operators. \
                FermionOperator(((l, 1), (k, 0), (j, 1), (i, 0)),
                                -double_amplitudes[i, j, k, l])
