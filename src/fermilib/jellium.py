...
def orbital_id(grid_length, grid_coordinates, spin=None):
  ...
  Args:
    ...
    grid_coordinates: List or tuple of ints giving coordinates of grid element.
...
def position_vector(position_indices, grid_length, length_scale):
  ...
  Returns:
    position_vector: A numpy array giving the position vector with dimensions.
  ...
  for grid_indices_a in itertools.product(range(grid_length),
    ...
    for grid_indices_b in itertools.product(range(grid_length),
      ...
      for omega_indices in itertools.product(range(grid_length),
        ...
        shifted_indices_d = [(grid_indices_a[i] - shifted_omega_indices[i]) %
                             grid_length for i in range(n_dimensions)]
        shifted_indices_c = [(grid_indices_b[i] + shifted_omega_indices[i]) %
                             grid_length for i in range(n_dimensions)]
        ...
        for spin_a in spins:
          for spin_b in spins:
            ...
            orbital_c = orbital_id(grid_length, shifted_indices_c, spin_b)
            orbital_d = orbital_id(grid_length, shifted_indices_d, spin_a)
  ...
  for grid_indices_a in itertools.product(range(grid_length),
    ...
    for grid_indices_b in itertools.product(range(grid_length),
      ...
      for momenta_indices in itertools.product(range(grid_length),
                                               repeat=n_dimensions):
        momenta = momentum_vector(momenta_indices, grid_length, length_scale)
        if momenta.any():
          coefficient += (numpy.cos(momenta.dot(differences)) *
  ...
  for grid_indices_a in itertools.product(range(grid_length),
    ...
    for grid_indices_b in itertools.product(range(grid_length),
      ...
      for momenta_indices in itertools.product(range(grid_length),
        ...
        momenta = momentum_vector(momenta_indices, grid_length, length_scale)
        if momenta.any():
          coefficient += (prefactor * numpy.cos(momenta.dot(differences)) /
                          momenta.dot(momenta))
...
def jordan_wigner_position_jellium(n_dimensions, grid_length,
  ...
  for k_indices in itertools.product(range(grid_length), repeat=n_dimensions):
  ...
  for p in range(n_qubits):
    for q in range(p + 1, n_qubits):
      ...
      for k_indices in itertools.product(range(grid_length),
        ...
        momenta = momentum_vector(k_indices, grid_length, length_scale)
        if momenta.any():
          term_coefficient += prefactor * momenta.dot(momenta) * numpy.cos(
              momenta.dot(differences))
