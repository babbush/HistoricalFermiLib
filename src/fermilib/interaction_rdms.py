...
class InteractionRDM(InteractionTensor):
  ...
  Attributes:
    ...
    two_body_tensor: The expectation values <a^\dagger_p a^\dagger_q a_r a_s>.
  ...
  def qubit_term_expectation(self, qubit_term):
    ...
    Args:
      qubit_term: QubitTerm instance to be evaluated on this InteractionRDM.
    """
    expectation = 0.
  ...
  def get_qubit_expectations(self, qubit_operator):
    ...
    for qubit_term in qubit_operator_expectations:
      if not qubit_term.is_identity():
        ...
        qubit_term.coefficient = self.qubit_term_expectation(qubit_term)
