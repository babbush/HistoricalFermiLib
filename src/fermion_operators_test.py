...
class HoppingOperatorsTest(unittest.TestCase):
  ...
  def test_hopping_operators(self):
    n14 = hopping_operator(self.n_qubits, 1, 4)
    t1 = FermionTerm(self.n_qubits, [(1, 1), (4, 0)])
    t4 = FermionTerm(self.n_qubits, [(4, 1), (1, 0)])
...
class NumberOperatorsTest(unittest.TestCase):
  ...
  def test_number_operator(self):
    ...
    self.assertEqual(n3, FermionTerm(self.n_qubits, [(3, 1), (3, 0)]))
...
class FermionTermsTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.term_a = FermionTerm(self.n_qubits, self.operators_a,
                              self.coefficient_a)
    self.term_b = FermionTerm(self.n_qubits, self.operators_b,
                              self.coefficient_b)
    self.normal_ordered_a = FermionTerm(self.n_qubits,
                                        [(4, 1), (3, 1), (1, 0)],
                                        self.coefficient_a)
    self.normal_ordered_b1 = FermionTerm(self.n_qubits, [(4, 0)],
                                         -self.coefficient_b)
    self.normal_ordered_b2 = FermionTerm(self.n_qubits,
                                         [(2, 1), (4, 0), (2, 0)],
                                         -self.coefficient_b)
  ...
  def test_eq_within_tol_same_ops(self):
    self.term1 = FermionTerm(2, [(1, 1)], coefficient=1.0)
    self.term2 = FermionTerm(2, [(1, 1)], coefficient=(1+9e-13))
    self.assertEqual(self.term1, self.term2)

  def test_eq_within_tol_diff_ops(self):
    self.term1 = FermionTerm(2, [(1, 1)], coefficient=9e-13)
    self.term2 = FermionTerm(2, [(1, 0)], coefficient=7e-13)
    self.assertEqual(self.term1, self.term2)
  ...
  def test_eq_different_nqubits_error(self):
    self.term1 = FermionTerm(1, operators=[], coefficient=1)
    self.term2 = FermionTerm(2, operators=[], coefficient=1)
    with self.assertRaises(local_terms.LocalTermError):
  ...
  def test_set_not_in(self):
    term1 = FermionTerm(5, operators=[(1, 1)])
    with self.assertRaises(local_terms.LocalTermError):
      term1[2] = [(2, 1)]
  ...
  def test_slicing_del(self):
    term1 = FermionTerm(10, [(i, 1) for i in range(10)])
    del term1[3:6]
  ...
  def test_add_different_nqubits_error(self):
    self.term1 = FermionTerm(5)
    self.term2 = FermionTerm(2)
    with self.assertRaises(local_terms.LocalTermError):
      self.term1 + self.term2
  ...
  def test_neg(self):
    expected = FermionTerm(self.n_qubits, self.operators_a,
                           -self.coefficient_a)
  ...
  def test_imul_localterm(self):
    ...
    expected = FermionTerm(self.n_qubits, expected_ops, expected_coeff)
  ...
  def test_mul_by_scalarzero(self):
    term1 = self.term_a * 0
    expected = FermionTerm(self.n_qubits, self.term_a.operators, 0.0)
  ...
  def test_mul_scalar(self):
    expected = local_terms.LocalTerm(self.n_qubits, self.operators_a,
                                     self.coefficient_a * (-3.+2j))
  ...
  def test_pow_square(self):
    ...
    expected = FermionTerm(self.n_qubits, self.operators_a + self.operators_a,
  ...
  def test_pow_high(self):
    high = self.term_a ** 10
    expected = FermionTerm(self.n_qubits, self.operators_a * 10,
  ...
  def test_abs_complex(self):
    term = FermionTerm(3, [(0, 1), (1, 0)], 2. + 3j)
    self.assertEqual(abs(term).coefficient, abs(term.coefficient))

  def test_len(self):
    term = FermionTerm(3, [(0, 0), (1, 1)], 2. + 3j)
  ...
  def test_normal_ordered_number(self):
    number_term2 = FermionTerm(self.n_qubits, [(2, 1), (2, 0)])
  ...
  def test_normal_ordered_number_reversed(self):
    n_term_rev2 = FermionTerm(self.n_qubits, [(2, 0), (2, 1)])
    number_term2 = FermionTerm(self.n_qubits, [(2, 1), (2, 0)])
    number_op2 = FermionOperator(self.n_qubits, number_term2)
  ...
  def test_normal_ordered_offsite(self):
    term = FermionTerm(self.n_qubits, [(3, 1), (2, 0)])
  ...
  def test_normal_ordered_offsite_reversed(self):
    term = FermionTerm(self.n_qubits, [(3, 0), (2, 1)])
    expected = -FermionTerm(self.n_qubits, [(2, 1), (3, 0)])
    op = FermionOperator(self.n_qubits, expected)
    self.assertEqual(op, term.normal_ordered())

  def test_normal_ordered_double_create(self):
    term = FermionTerm(self.n_qubits, [(2, 0), (3, 1), (3, 1)])
    expected = FermionTerm(self.n_qubits, [], 0.0)
    op = FermionOperator(self.n_qubits, expected)
    self.assertEqual(op, term.normal_ordered())

  def test_normal_ordered_multi(self):
    term = FermionTerm(self.n_qubits, [(2, 0), (1, 1), (2, 1)])
    ordered_212 = -FermionTerm(self.n_qubits, [(2, 1), (1, 1), (2, 0)])
    ordered_1 = -FermionTerm(self.n_qubits, [(1, 1)])
  ...
  def test_normal_ordered_triple(self):
    term_132 = FermionTerm(self.n_qubits, [(1, 1), (3, 0), (2, 0)])
    op_132 = FermionOperator(self.n_qubits, term_132)

    term_123 = FermionTerm(self.n_qubits, [(1, 1), (2, 0), (3, 0)])
    op_123 = FermionOperator(self.n_qubits, term_123)

    term_321 = FermionTerm(self.n_qubits, [(3, 0), (2, 0), (1, 1)])
  ...
  def test_jordan_wigner_transform_raise3(self):
    raising = FermionTerm(self.n_qubits, [(3, 1)]).jordan_wigner_transform()
  ...
  def test_jordan_wigner_transform_raise1(self):
    raising = FermionTerm(self.n_qubits, [(1, 1)]).jordan_wigner_transform()
    self.assertEqual(len(raising), 2)
  ...
  def test_jordan_wigner_transform_lower3(self):
    lowering = FermionTerm(self.n_qubits, [(3, 0)]).jordan_wigner_transform()
    self.assertEqual(len(lowering), 2)

    correct_operators_x = [(0, 'Z'), (1, 'Z'), (2, 'Z'), (3, 'X')]
    correct_operators_y = [(0, 'Z'), (1, 'Z'), (2, 'Z'), (3, 'Y')]
    qtermx = QubitTerm(self.n_qubits, correct_operators_x, 0.5)
    qtermy = QubitTerm(self.n_qubits, correct_operators_y, 0.5j)
  ...
  def test_jordan_wigner_transform_lower2(self):
    lowering = FermionTerm(self.n_qubits, [(2, 0)]).jordan_wigner_transform()
    self.assertEqual(len(lowering), 2)
  ...
  def test_jordan_wigner_transform_lower1(self):
    lowering = FermionTerm(self.n_qubits, [(1, 0)]).jordan_wigner_transform()
    self.assertEqual(len(lowering), 2)
  ...
  def test_jordan_wigner_transform_lower0(self):
    lowering = FermionTerm(self.n_qubits, [(0, 0)]).jordan_wigner_transform()
    self.assertEqual(len(lowering), 2)
  ...
  def test_jordan_wigner_transform_raise3lower0(self):
    ...
    term = FermionTerm(self.n_qubits,
  ...
  def test_bravyi_kitaev_transform(self):
    ...
    lowering = FermionTerm(self.n_qubits, [(3, 0)]).bravyi_kitaev_transform()
    raising = FermionTerm(self.n_qubits, [(3, 1)]).bravyi_kitaev_transform()
    self.assertEqual(len(raising), 2)
    self.assertEqual(len(lowering), 2)
    ...
    for index in range(n_qubits):
      operator = FermionTerm(n_qubits, [(index, 0)]).bravyi_kitaev_transform()
    ...
    lowering = FermionTerm(n_qubits, [(9, 0)]).bravyi_kitaev_transform()
    raising = FermionTerm(n_qubits, [(9, 1)]).bravyi_kitaev_transform()
  ...
  def test_jw_ccr_offsite_even_ca(self):
    c2 = FermionTerm(5, [(2, 1)])
    a4 = FermionTerm(5, [(4, 0)])
  ...
  def test_is_molecular_term_updown(self):
    term = FermionTerm(self.n_qubits, [(2, 1), (4, 0)])
    self.assertTrue(term.is_molecular_term())

  def test_is_molecular_term_downup(self):
    term = FermionTerm(self.n_qubits, [(2, 0), (4, 1)])
    self.assertTrue(term.is_molecular_term())

  def test_is_molecular_term_downup_badspin(self):
    term = FermionTerm(self.n_qubits, [(2, 0), (3, 1)])
    self.assertFalse(term.is_molecular_term())

  def test_is_molecular_term_three(self):
    term = FermionTerm(self.n_qubits, [(0, 1), (2, 1), (4, 0)])
    self.assertFalse(term.is_molecular_term())

  def test_is_molecular_term_four(self):
    term = FermionTerm(self.n_qubits, [(0, 1), (2, 0), (1, 1), (3, 0)])
    self.assertTrue(term.is_molecular_term())
...
class FermionOperatorsTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.term_b = FermionTerm(self.n_qubits, self.operators_b,
                              self.coefficient_b)
    self.term_c = FermionTerm(self.n_qubits, self.operators_c,
                              self.coefficient_c)
  ...
  def test_init_list_protection(self):
    ...
    terms = [FermionTerm(self.n_qubits, operators1, coeff1,)]

    operator1 = FermionOperator(self.n_qubits, terms)
    terms.append((3, 1))

    expected_term = FermionTerm(self.n_qubits, operators1, coeff1)
  ...
  def test_neg(self):
    term = FermionTerm(self.n_qubits, self.operators_a, -self.coefficient_a)
  ...
  def test_mul_by_zero_fermionterm(self):
    zero_term1 = FermionTerm(self.n_qubits, [(0, 1)], 0.0)
  ...
  def test_mul_by_identity_term(self):
    identity_term = FermionTerm(self.n_qubits)
  ...
  def test_bk_jw_number_operator(self):
    # Check if number operator has the same spectrum in both representations
    n_qubits = 1
  ...
  def test_bk_jw_number_operator_scaled(self):
    # Check if number operator has the same spectrum in both representations
    n_qubits = 1
    no = number_operator(n_qubits, 0, coefficient=2)  # eigenspectrum (0,2)
    jw_no = no.jordan_wigner_transform()
    bk_no = no.bravyi_kitaev_transform()
  ...
  def test_bk_jw_majoranas(self):
    ...
    a = FermionTerm(n_qubits, [(1, 0)])
    a_dag = FermionTerm(n_qubits, [(1, 1)])
